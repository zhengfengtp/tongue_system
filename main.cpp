...
int cv_tonRegion(Mat& img, Mat& dst){
	...
	//imwrite("mor_tonRGB.jpg", mor_tonRGB);
	...
	//imwrite("mor_entropy.jpg", entropy);
	...
	//imwrite("mor_otsu_polar.jpg", mor_otsu_polar);
...
int cv_findCon(Mat& img, Mat& img_feature, Mat& img_mask){
	...
	Moments mom1 = moments(hull_mode);
	double hu1[7];
	HuMoments(mom1, hu1);
	for(int i = 0; i < 7; i++) cout << hu1[i] << "##";
...
int cv_grayWorld(Mat& img, Mat& dst, int method){ //0：green 1：min 2：max 3：mid 4：改进gamma
	...
	case 3:
		...
		alpha_r = 1.09 * avg_mid / avg_r;
		alpha_b = 0.94 * avg_mid / avg_b;
		alpha_g = 0.956 * avg_mid / avg_g;
...
int cv_dcstretch(Mat& src, Mat& dst)
{
	...
	int row_rgb = rgb_g.rows, col_rgb = rgb_g.cols;
	...
	Mat matrixT, tmp;
	pow(evalue, -1/2, tmp);
	matrixT = evector.t() * sum(tmp).val[0] / 3  * evector;
	...
	for(int i = 0; i < row_a; i++)
	{
		...
		for(int j = 0; j < col_a; j++)
		{
			channels.at(i).at<uchar>(j/col_rgb, j%col_rgb) = Ma[j];	
		}
	}
	merge(channels, dst);
	return 0;
...
int main()
{
	string p_path = "./test/";  //./Tongue_Image/20131019/
	...
	for(string file_name; getline(in, file_name); )
	{
		...
		//imshow("src", img);
		//Mat img = imread("./test/pos_109.jpg");
		...
		imshow("dstrhsv8u", dstrhsv8u); //去相关和拉伸
